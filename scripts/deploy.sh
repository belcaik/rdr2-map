...
if [[ $CONTAINER_ENGINE = podman ]]; then
  ...
  remote_command "$REMOTE_CD && $REMOTE_COMPOSE run --rm --user 0 -v ./import:/import map sh -ec $(shell_quote 'mkdir -p /data /import && chown -R 1000:1000 /data && chmod -R u+rwX,a+rX /import')" || die 'Docker directory ownership setup failed'
fi
if [[ -n $LOCAL_DATASET_DIR ]]; then
...
if [[ $CONTAINER_ENGINE = podman ]]; then
  ...
  health_loop='for attempt in $(seq 1 30); do container=$('"$REMOTE_COMPOSE"' ps -q map 2>/dev/null); if [ -n "$container" ] && podman healthcheck run "$container" >/dev/null 2>&1; then exit 0; fi; sleep 2; done; exit 1'
